...
function getNextFriday(date) {
  ...
  const fridayUTCNum = 5;
  const curDay = d.getUTCDay();

  let daysToTheNextFriday = 0;
  if (curDay >= fridayUTCNum) {
    daysToTheNextFriday = 12 - curDay;
  } else {
    daysToTheNextFriday = fridayUTCNum - curDay;
  }
  const fridayDate = curDate + daysToTheNextFriday;
  const newDate = new Date(d.setDate(fridayDate));
  return newDate;
}
...
function isDateInPeriod(date, period) {
  const newArr = date.split('-').map((dateItem) => {
    return Number(dateItem);
  });

  const startDateArr = period.start.split('-').map((dateItem) => {
    return Number(dateItem);
  });
  const endDateArr = period.end.split('-').map((dateItem) => {
    return Number(dateItem);
  });

  const utc1 = Date.UTC(...newArr);
  const startUtc = Date.UTC(...startDateArr);
  const endUtc = Date.UTC(...endDateArr);

  return utc1 >= startUtc && utc1 <= endUtc;
}
...
function formatDate(dateEntry) {
  const d = new Date(dateEntry);
  const date = d.getUTCDate();
  const month = d.getMonth() + 1;
  const year = d.getFullYear();
  const timeOptions = { hour12: true, timeZone: 'UTC' };
...
function getCountWeekendsInMonth(month, year) {
  const daysInAMonth = getCountDaysInMonth(month, year);
  const sunday = 0;
  const saturday = 6;
  let weekends = 0;
...
function getWeekNumberByDate(/* d */) {
  ...
  // const totalGoneDays = (d - janDate) / (1000 * 60 * 60 * 24);
  // const firstDayInAYear = d.getDay();
  // console.log(new Date(year, 0, 1).getDay());
  // let week = 0;
  // if (firstDayInAYear !== 0) {
  //   week = 1;
  // }
  // week += Math.floor(totalGoneDays / 7);
  // // for (let i = 1; i <= month; i += 1) {
  // //   totalGoneDays += getCountDaysInMonth(i, year);
  // //   console.log(getCountDaysInMonth(i, year));
  // // }
  // // totalGoneDays -= getCountDaysInMonth(month, year) - date;
  // return week;
...
function getNextFridayThe13th(date) {
  ...
  return getNextFridayThe13th(new Date(year, month + 1));
}
